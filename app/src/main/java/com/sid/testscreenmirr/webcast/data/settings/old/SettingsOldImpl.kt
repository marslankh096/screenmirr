package  com.sid.testscreenmirr.webcast.info.dvkr.screenstream.data.settings.old
...
class SettingsOldImpl(private val preferences: Preferences) : SettingsOld {
    ...
    override var notifySlowConnections: Boolean
            by bindPreference(preferences,
                SettingsOld.Key.NOTIFY_SLOW_CONNECTIONS,
                SettingsOld.Default.NOTIFY_SLOW_CONNECTIONS
            )
    ...
    override var lastIAURequestTimeStamp: Long
            by bindPreference(preferences,
                SettingsOld.Key.LAST_IAU_REQUEST_TIMESTAMP,
                SettingsOld.Default.LAST_IAU_REQUEST_TIMESTAMP
            )
    ...
    private val changeListenerSet = Collections.synchronizedSet(HashSet<SettingsOldReadOnly.OnSettingsChangeListener>())
    ...
    override fun registerChangeListener(listener: SettingsOldReadOnly.OnSettingsChangeListener) {
        synchronized(changeListenerSet) {
    ...
    override fun unregisterChangeListener(listener: SettingsOldReadOnly.OnSettingsChangeListener) {
        synchronized(changeListenerSet) {
